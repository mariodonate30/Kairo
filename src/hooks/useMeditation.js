...
// sesión de meditación registrada. Si la última sesión es más antigua que ayer,
// la racha se considera rota (0).
function currentStreak(dates) {
  if (dates.size === 0) return 0

  // La racha sigue viva si hubo meditación hoy o ayer; contamos desde ahí.
  let cursor
...
export function useMeditation() {
  ...
  const load = useCallback(async () => {
    ...
    if (error) {
      setError('No se han podido cargar tus sesiones de meditación.')
    } else {
      setSessions(data || [])
    }
  ...
  const stats = useMemo(() => {
    ...
    for (const session of sessions) {
      ...
      if (session.date >= weekStart) {
        weekMinutes += session.duration_minutes
        weekSessions += 1
      }
    }
  ...
  // Registra una sesión de meditación completada. Solo se llama cuando el
  // temporizador llega a cero, así que cada registro es una sesión entera.
